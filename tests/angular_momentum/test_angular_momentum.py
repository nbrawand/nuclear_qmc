...
def test_L_sqrd_of_orbital_wfc():
    ...
    particle_pairs, _, _, _, _ = get_system_arrays(
    ...
    key, psi, psi_params = build_wave_function(key, n_neu, n_pro, 1, 1
                                               ...
                                               , coefficients=jnp.array([1.]), confining_factor=0.
                                               )
    ...
    computed = jnp.round(computed, 8)
    expected = jnp.zeros(shape=(2, 4), dtype=jnp.complex64)
...
def test_L_sqrd_of_complete_wfc_2H():
    ...
    n_pro = 1
    r_coords = jnp.array(np.random.random(size=(n_neu + n_pro, 3)))
    particle_pairs, particle_triplets, spin_exchange_indices, isospin_exchange_indices, isospin_binary_representation = get_system_arrays(
        n_pro, n_neu, also_return_binary_representation=True)
