...
def get_rotate_r(r_coords, theta, ith_particle):
    cos = jnp.cos(theta)
    sin = jnp.sin(theta)
...
def rotate_z(func_3d, r_coords, theta, ith_particle):
    r_coords_prime = get_rotate_r(r_coords, theta, ith_particle)
...
def finite_diff_theta(func, r_coords, ith_particle):
    dtheta = 0.0001
    rot_func_r = rotate_z(func, r_coords, dtheta, ith_particle) - rotate_z(func, r_coords, -dtheta, ith_particle)
    rot_func_r /= 2.0 * dtheta
    return rot_func_r


def get_total_angular_momentum_z(func, r_coords):
    particles = jnp.arange(r_coords.shape[0])
    partials = vmap(finite_diff_theta, in_axes=(None, None, 0))(func, r_coords, particles)
