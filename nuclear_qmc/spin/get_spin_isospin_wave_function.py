...
def get_spin_isospin_wave_function(n_protons, n_neutrons, include_isospin=True):
    ...
    if include_isospin:
        wave_function = jnp.zeros(shape=(n_isospin_states, n_spin_states), dtype=jnp.complex64)
    else:
        wave_function = jnp.zeros(shape=n_spin_states, dtype=jnp.complex64)
